...
class LoginToken:
    """登录 Token 类"""
    def __init__(self, token: str, headers: Dict[str, str], expires_at: Optional[float] = None):
        self.token = token
        self.headers = headers
        self.expires_at = expires_at

    def __repr__(self):
        return f"LoginToken(token={self.token[:20]}..., expires_at={self.expires_at})"
...
def _perform_login(
    ...
    Returns:
        LoginToken 对象，如果登录失败则返回 None
    """
    logger.info(f"开始为 {base_url_alias} 执行登录...")

    # 加载登录用例
    test_cases = _load_login_file(base_url_alias, env_name, env_config)
    if not test_cases:
        logger.error(f"未找到 {base_url_alias} 的登录用例")
        return None
    ...
    if not base_url:
        logger.error(f"未找到有效的 base_url，别名: {base_url_alias}")
        return None
    ...
    if case_url and case_url.startswith("http"):
        ...
        logger.error(f"登录用例未配置 url 或 path")
        return None
    ...
    logger.info(f"发送登录请求: {method} {url}")
    ...
    try:
        ...
        if assertions:
            ...
            for result in results:
                if not result.passed:
                    logger.error(f"登录断言失败: {result.message}")
                    logger.error(f"  断言类型: {result.assertion_type}, 实际值: {result.actual}, 期望值: {result.expected}")
                    return None
                else:
        ...
        if not extractions:
            logger.error(f"登录用例未配置 extractions，无法提取 token")
            return None
        ...
        if not token:
            logger.error(f"未能从响应中提取到 token")
            return None

        logger.info(f"✅ {base_url_alias} 登录成功，token: {token[:20]}...")
        ...
        logger.exception(f"{base_url_alias} 登录过程中发生异常")
        return None
...
def get_service_headers(
    ...
    if not force_refresh and cache_key in _login_cache:
        ...
        if login_token:
            logger.info(f"✅ 从缓存获取 {service_type} 的请求头")
            return login_token.headers
        else:
            logger.warning(f"⚠️  {service_type} 的登录已缓存但失败，使用默认 headers")
    ...
    if login_token:
        ...
        logger.warning(f"⚠️  {service_type} 登录失败，使用默认 headers")
...
def auth_headers(request, http_client, env_config, login_cache) -> Dict[str, str]:
    ...
    if service_type:
        ...
        # 检查服务是否在白名单中（如果配置了 required_services）
        auth_config = env_config.get('auth', {})
        required_services = auth_config.get('required_services', [])

        if required_services and service_type not in required_services:
            logger.warning(f"⚠️  {service_type} 不在 required_services 白名单中，跳过登录")
        else:
            # 执行懒加载登录
